...
function sendMenu($bot,$chat_id,$menu,$keyboard = null,$additional = "",$msg_id = false) {
    $main_array = [];
    if (isset($menu[0]['url_buttons']) && $keyboard == null) {
        $url_buttons = json_decode($menu[0]['url_buttons'], true);
        foreach ($url_buttons as $button) {
            array_push($main_array, $button);
        }
    ...
    if (!empty($main_array)) $keyboard = new \TelegramBot\Api\Types\Inline\InlineKeyboardMarkup($main_array);
    $caption = null;
    if (isset($menu[0]['caption'])) $caption = $menu[0]['caption'].$additional;
    ...
    if (isset($menu[0]['text']) && !is_null($menu[0]['text'])) {
        ...
        try {
            if ($msg_id >= 1) {
                return $bot->editMessageText($chat_id,$msg_id,$menu[0]['text'] . $additional,"html",false,$keyboard);
            }else {
                return $bot->sendMessage($chat_id, $menu[0]['text'] . $additional, "html", false, null, $keyboard);
            }
        } catch (TelegramBot\Api\HttpException $e) {
            return "banned";
        }
    ...
    if (isset($menu[0]['document']) && !is_null($menu[0]['document'])) {
        $replyToMessageId = null;
        $disableNotification = false;

        try {
            return $bot->sendDocument($chat_id,$menu[0]['document'],$caption,$replyToMessageId,$keyboard,$disableNotification,"html");
        } catch (TelegramBot\Api\HttpException $e) {
            return "banned";
        }
    }
    if (isset($menu[0]['voice']) && !is_null($menu[0]['voice'])) {
        $replyToMessageId = null;
        $disableNotification = false;

        try {
            return $bot->sendVoice($chat_id,$menu[0]['voice'],null,$replyToMessageId,$keyboard,$disableNotification,"html");
        } catch (TelegramBot\Api\HttpException $e) {
            return "banned";
        }
    }
    if (isset($menu[0]['media_group_id']) && !is_null($menu[0]['media_group_id'])) {
        // todo
    }
}