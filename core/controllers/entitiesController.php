...
class EntityDecoder
{
    ...
    public function decode($message): string
    {
        ...
        if (isset($this->caption)) $this->text = $this->caption;
        if (isset($this->caption)) $this->entities = $message->getCaptionEntities();
        ...
        $arrayText = $this->splitCharAndLength($this->text);
        $finalText = "";
...
//                    }
                ...
                case 'text_mention':
                {
                    $startString .= '<a href="tg://user?id='.$entity->getUser()->getId().'">';
                    break;
                }
                case 'text_link':
                {
                    $startString .= '<a href="'.$entity->getUrl().'">';
                    break;
                }
            }
        }
    ...
    protected function checkForEntityStart($pos)
    {
        ...
        {
            if($entity->getOffset() == $pos)
            {
                if(in_array($entity->getType(), array('bold', 'italic', 'code', 'pre', 'text_mention', 'text_link', 'strikethrough', 'underline')))
                {
                    $entities[] = $entity;
                }
            }
    ...
    protected function getEntityStopString($entity)
    {
        ...
        if($this->style == 'Markdown')
        {
            switch($entity->getType())
            {
                ...
                case 'text_mention':
                {
                    $stopString = '](tg://user?id='.$entity->getUser()->getId().')';
                    break;
                }
                case 'text_link':
                {
                    $stopString = ']('.$entity->getUrl().')';
                    break;
                }
            }
        }
...
//                    }
    ...
    protected function checkForEntityStop($pos)
    {
        ...
        foreach($this->entities as $entity)
        {
            if($entity->getOffset() + $entity->getLength() == $pos)
            {
                if(in_array($entity->getType(), array('bold', 'italic', 'code', 'pre', 'text_mention', 'text_link', 'strikethrough', 'underline')))
                {
                    $entities[] = $entity;
    ...
    protected function checkMarkdownV2AmbiguousEntities(&$entitiesToCheck)
    {
        ...
        {
            if($ec->getType() == 'italic' || $ec->getType() == 'underline')
            {
                $foundIndex++;
            }
        }
